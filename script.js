...
const BUILD_CONFIG = { unlimitedSessions: false };
...
function addHistory(text,type='event',detail='',meta={}) {
  const row={id:uid(),text,type,detail,...meta}; state.history.push(row); lastAnimatedHistoryId=row.id; renderHistory();
}
function setFishHistoryEaten(fish, eaten=true) {
  state.history.forEach(row=>{
    if (row.type==='fish' && row.fishId===fish.id) row.eaten=eaten;
  });
}
...
function processDebuff(forcedName=null) {
  ...
  if (name==='Касатка') {
    state.fish.filter(f=>!f.removed&&f.weight>=5.5).forEach(f=>{f.removed=true;state.eaten.push(f);state.fishLostToDebuffs=true;setFishHistoryEaten(f,true);});
...
function activateMegalodon() {
  ...
  [...state.stolen,...state.eaten].forEach(f=>{f.removed=false;f.weight=f.originalWeight;if(!state.fish.includes(f))state.fish.push(f);setFishHistoryEaten(f,false);});
...
function renderHistory() {
  ...
  $('historyList').innerHTML=state.history.map((h,i)=>{const icon=h.type==='fish'&&h.eaten?'💀':(icons[h.type]||'');return `<li data-history-id="${h.id}" class="history-item type-${h.type}${h.eaten?' is-eaten':''}${h.id===lastAnimatedHistoryId?' is-new':''}"><strong>${i+1}. ${icon} ${h.text}</strong>${h.detail?`<small>${h.detail}</small>`:''}</li>`;}).join('');
